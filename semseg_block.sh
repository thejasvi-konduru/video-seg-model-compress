...
#SBATCH --output=log_outputs/block_outputs/semseg_out_drn_d_22%J.out
...
batch_size=12
input_size='512X512'
pconfig_path=sparse_experiments/block_cityscapes_drn_d_22/sparse_srmbrep_cityscapes_drn_d_22_srmbrep_-1x-1_-1x-1_4x4_75.00-RAMANUJAN_0.00-RAMANUJAN_50.00_collapse_repetitive/config.json
tensorboard_path=runs/cityscapes_block_drn_d_22/
# # # base_model=base_models/resnet50-19c8e357.pth
# # ename=${dataset}_${arch}_${epochs}

exp_dir=sparse_experiments/block_cityscapes_drn_d_22/sparse_srmbrep_cityscapes_drn_d_22_srmbrep_-1x-1_-1x-1_4x4_75.00-RAMANUJAN_0.00-RAMANUJAN_50.00_collapse_repetitive
...
python semantic_seg.py train -d=${scratch_dir}/cityscapes/cityscapes \
         --dataset=${dataset} \
         --arch=${arch} \
         --exp_dir=${exp_dir} \
         --mc_pruning \
         ...
         --pr-static \
         --lr=${lr} \
		 --input_size=${input_size} --sparsity=${sp} \
		 --tensorboard_path=${tensorboard_path} \
         --epochs=${epochs} -b=${batch_size} | tee=${exp_dir}/log.txt