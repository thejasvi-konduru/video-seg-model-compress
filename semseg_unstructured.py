...
def parse_args():
    ...
    parser.add_argument('-crop', '--crop_size', default='1024X768', type=str)
    ...
    parser.add_argument('--dist-url', default='tcp://127.0.0.1:2323', type=str,
    ...
    # Pruning related
    parser.add_argument('-u',"--sparse_type", type = str, default="unstructured", help="Enable model compression using pruning")
...
def main():
    ...
    #     model = torch.nn.DataParallel(single_model, device_ids= args.gpu).to(device)
    ...
    if args.sparse_type=="unstructured":
        ...
        print("Conv count {}".format(count))
        print(dict(model.named_buffers()).keys())
        
        num_zeros, num_elements, global_sparsity = measure_global_sparsity(
            model,
            weight=True,
            bias=False,
        ...
        print("Num zeros: {}, Num elements: {}, Global sparsity:{}".format(num_zeros, num_elements, global_sparsity))
    ...
    if args.epochs == 400: #imagenet
        ...
        ## WRN configuration
        milestones = [60,120,180,240,300,350,420,450]
        gamma = 0.1
    ...
    if args.cmd == 'train':
        ...
        if args.resume:
            ...
            # if args.sparse_type=="unstructured":
    	    writer = SummaryWriter("runs/"+args.dataset+"_"+args.arch+"_"+crop_size+"_"+args.sparse_type+str(sparsity)+"_"+str(args.epochs))
            # else:
    	    #     writer = SummaryWriter("runs/"+args.dataset+"_"+args.arch+"_"+crop_size+"_"+args.sparse_type+str(sparsity)+"_"+str(args.epochs))
        ...
        if not args.resume:
            if args.sparse_type=="unstructured":  
                save_path = args.exp_dir+"_"+args.sparse_type+str(sparsity)+"_e"+str(args.epochs)
            else:
