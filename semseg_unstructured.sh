...
#SBATCH --output=log_outputs/semseg_unstructured_drn_d_54%J.out
...
ename=${sparse_type}_${dataset}_${arch}_${epochs}_${crop_size}_${sparsity}_${batch_size}
...
scratch_dir=/ssd_scratch/cvit/furqan
...
# python semseg_baseline.py -d=${scratch_dir}/cityscapes/cityscapes train -c 19 --epochs=${epochs} --crop_size=${crop_size} --lr=${lr} -b=${batch_size} --exp_dir=experiments/${ename} | tee experiments/semseg_baseline_drn_d_54_1024X768.txt

python semseg_unstructured.py --epochs=${epochs} \
	   train -d=${scratch_dir}/cityscapes/cityscapes/ \
	    -c 19 --lr=${lr} --arch=${arch} \
		--crop_size=${crop_size} -b=${batch_size} \
		--sparsity=${sparsity} -u=${sparse_type} \
		--exp_dir=sparse_experiments/${ename} | tee sparse_experiments/${ename}_log_501.txt