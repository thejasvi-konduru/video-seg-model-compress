...
class DRNSeg(nn.Module):
	def _init_(self, model_name, classes, pretrained_model=None,
				 pretrained=True, use_torch_up=False):
		super(DRNSeg, self)._init_()
		model = drn._dict_.get(model_name)(
...
def FrameCapture(video_path, model):
	...
	count = 0
	success = 1
	images = torch.zeros((30, 3, 300, 300))
	raw_images = []
...
class ImageView(pg.ImageView):
	...
	def _init_(self, *args, **kwargs):
		pg.ImageView._init_(self, *args, **kwargs)
...
arch = "drn_d_22"
classes = 19
pretrained_model = "drn_d_22_cityscapes.pth"
model = DRNSeg(arch, classes, pretrained_model,
					pretrained=True)
